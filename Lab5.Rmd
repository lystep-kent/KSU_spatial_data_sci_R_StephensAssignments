...
##Setup
```{r}
data <-  sf::read_sf("./data/ACS_2020_5YR_COUNTY.gdb", layer = "ACS_2020_5YR_COUNTY")
glimpse(data)

datable <- sf::read_sf("./data/ACS_2020_5YR_COUNTY.gdb", layer = "X01_AGE_AND_SEX") %>%
...
```{r}
states <- datajoin %>% group_by(.,STATEFP) %>% dplyr::filter(., STATEFP %in% c("41","53","16","30"))
...
mordat <- moran.plot(state.projected$under5, rowstand, zero.policy=TRUE, plot=TRUE, labels=TRUE, return_df=TRUE)
```
LISA Data
```{r}
